...
//         fm = getSupportFragmentManager();  // **When would you use this instead?? A: __________________
    ...
    public void showFrag1() {
        System.out.println("showFrag1");
        f1 = (Frag_One) fm.findFragmentByTag("tag1");   //what should we do if f1 doesn't exist anymore?  How do we check and how do we fix? Recreate new frag_One();
        FragmentTransaction ft = fm.beginTransaction();
        if (f1 == null){
            System.out.println("showFrag1() null");
            f1 = new Frag_One();
    ...
    public void showFrag2() {
        ...
        ft.addToBackStack("myFrag2");  //Q: What is the back stack and why do we do this? This ensure if the back button is pressed, the previous fragment will show.
        ft.commit();
    }


    public void showFrag3() {
        System.out.println("showFrag3");
        f3 = (Frag_Three) fm.findFragmentByTag("tag3");
        FragmentTransaction ft = fm.beginTransaction();  //Create a reference to a fragment transaction.
        if (f3 == null){
            System.out.println("fragment 3 null");
            f3 = new Frag_Three();
            ft.replace(R.id.FragLayout, f3, "tag3");
        }
        //ft.detach(f1);   //what would happen if f1, f2, or f3 were null?  how would we check and fix this? we could fix it by checking if they are null and create a new frag.
        //ft.detach(f2);
        //ft.attach(f3);
        else {
